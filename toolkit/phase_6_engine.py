...
def build(skill: dict[str, str], payload: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if 'transcrip' in low or 'resumen de subtítulos' in low:
        flow = '1. Detectar pista o transcribir localmente.\n2. Conservar marcas de tiempo.\n3. Revisar nombres y términos.\n4. Generar resumen separado de la transcripción.\n5. Eliminar o proteger material sensible.'
        return f"# Transcripción y resumen\n\n{sec('Material requerido', text or '[Ruta a audio o video autorizado]')}{sec('Flujo', flow)}", ['No se procesó ningún archivo ni se llamó faster-whisper automáticamente.']
    if 'storyboard' in low or 'video' in low or 'cine' in low or 'anuncio' in low or 'campaña' in low or 'shorts' in low or 'clipper' in low or 'ugc' in low or 'drón' in low or 'dron' in low:
        ...
        storyboard_block = '```json\n' + json.dumps(data, ensure_ascii=False, indent=2) + '\n```'
        return f"# {name}\n\n{sec('Storyboard', storyboard_block)}{sec('Continuidad', 'Mantener identidad, vestuario, iluminación, lente, relación de aspecto y audio consistentes entre escenas.')}{sec('Exportación', 'Definir codec, resolución, fps, bitrate, subtítulos, miniatura y plataforma antes de renderizar.')}", ['No se generó video; revisar storyboard y derechos antes de renderizar.']
    if 'prompt' in low or 'director' in low or 'seedance' in low or 'kling' in low or 'modelo' in low:
        parameters = 'Duración: [segundos]\nRelación: [16:9 / 9:16 / 1:1]\nMovimiento: [describir]\nContinuidad: [describir]'
        return f"# Prompt audiovisual\n\n{sec('Idea', text or '[Idea de video]')}{sec('Prompt estructurado', 'Sujeto + acción + entorno + composición + cámara + iluminación + textura + ritmo + audio + restricciones negativas.')}{sec('Parámetros', parameters)}{sec('Seguridad', 'No imitar personas, voces, marcas o estilos protegidos sin permiso; no presentar el prompt como video generado.')}", ['Adaptar al proveedor solo tras confirmar modelo, política y credenciales.']
    ...
    if 'imagemagick' in low or 'restauración' in low or 'foto' in low:
        ...
        return f"# Flujo de imagen\n\n{sec('Entrada', text or '[Ruta de imagen autorizada]')}{sec('Operaciones', 'Inspección de formato y metadatos, copia de seguridad, transformación, exportación y revisión visual.')}{sec('Comando propuesto', command)}{sec('Privacidad', 'Revisar EXIF, rostros, identidad y permisos antes de compartir.')}", ['El comando es una propuesta; no se modificó ninguna imagen.']
