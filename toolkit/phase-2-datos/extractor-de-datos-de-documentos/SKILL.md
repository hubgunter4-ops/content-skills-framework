---
name: extractor-de-datos-de-documentos
description: Motor de análisis y procesamiento de Word, PDF y PowerPoint que extrae tablas, números, gráficos y formato de los documentos. Convierte informes, contratos y presentaciones no estructurados en datos estructurados que puedes analizar y resumir en varios archivos.
---
...
Motor de análisis y procesamiento de Word, PDF y PowerPoint que extrae tablas, números, gráficos y formato de los documentos. Convierte informes, contratos y presentaciones no estructurados en datos estructurados que puedes analizar y resumir en varios archivos. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/extractor-de-datos-de-documentos/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `extractor-de-datos-de-documentos` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/extractor-de-datos-de-documentos/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/extractor-de-datos-de-documentos/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/extractor-de-datos-de-documentos/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/extractor-de-datos-de-documentos/run.py \
  --input toolkit/phase-2-datos/extractor-de-datos-de-documentos/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Extractor de Datos de Documentos` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Extractor de Datos de Documentos",
