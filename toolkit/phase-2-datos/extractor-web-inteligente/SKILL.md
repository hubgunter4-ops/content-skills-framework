---
name: extractor-web-inteligente
description: Este asistente inteligente de extracción web elude automáticamente las protecciones anti-bot como Cloudflare para extraer datos de páginas y contenido de artículos. Ofrece un flujo de trabajo completo con selección inteligente de extractores y generación de scripts en Python.
---

# Extractor Web Inteligente

## Propósito

Este asistente inteligente de extracción web elude automáticamente las protecciones anti-bot como Cloudflare para extraer datos de páginas y contenido de artículos. Ofrece un flujo de trabajo completo con selección inteligente de extractores y generación de scripts en Python. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/extractor-web-inteligente/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `extractor-web-inteligente` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/extractor-web-inteligente/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/extractor-web-inteligente/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/extractor-web-inteligente/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/extractor-web-inteligente/run.py \
  --input toolkit/phase-2-datos/extractor-web-inteligente/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Extractor Web Inteligente` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Extractor Web Inteligente",
