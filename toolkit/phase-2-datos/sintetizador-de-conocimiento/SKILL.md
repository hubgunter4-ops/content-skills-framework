...
description: Sintetiza los resultados de búsqueda de múltiples fuentes en respuestas coherentes y sin duplicados, con una clara atribución de fuentes y puntuación de confianza. Pondera la actualidad y la autoridad para mostrar la información más fiable.
---
...
Sintetiza los resultados de búsqueda de múltiples fuentes en respuestas coherentes y sin duplicados, con una clara atribución de fuentes y puntuación de confianza. Pondera la actualidad y la autoridad para mostrar la información más fiable. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/sintetizador-de-conocimiento/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `sintetizador-de-conocimiento` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/sintetizador-de-conocimiento/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/sintetizador-de-conocimiento/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/sintetizador-de-conocimiento/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/sintetizador-de-conocimiento/run.py \
  --input toolkit/phase-2-datos/sintetizador-de-conocimiento/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Sintetizador de Conocimiento` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Sintetizador de Conocimiento",
