---
name: control-de-calidad-de-arn-de-celula-unica
description: Herramienta que realiza el control de calidad en conjuntos de datos de RNA-seq de célula única .h5ad o .h5 siguiendo las mejores prácticas de scverse/scanpy, aplicando filtrado basado en MAD para identificar y eliminar células de baja calidad. Genera visualizaciones exhaustivas y métricas de resumen para ayudarle a evaluar la calidad de los datos, elegir umbrales de filtrado y producir objetos limpios para análisis posteriores.
---

# Control de Calidad de ARN de Célula Única

## Propósito

Herramienta que realiza el control de calidad en conjuntos de datos de RNA-seq de célula única .h5ad o .h5 siguiendo las mejores prácticas de scverse/scanpy, aplicando filtrado basado en MAD para identificar y eliminar células de baja calidad. Genera visualizaciones exhaustivas y métricas de resumen para ayudarle a evaluar la calidad de los datos, elegir umbrales de filtrado y producir objetos limpios para análisis posteriores. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/control-de-calidad-de-arn-de-celula-unica/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `control-de-calidad-de-arn-de-celula-unica` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/control-de-calidad-de-arn-de-celula-unica/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/control-de-calidad-de-arn-de-celula-unica/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/control-de-calidad-de-arn-de-celula-unica/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/control-de-calidad-de-arn-de-celula-unica/run.py \
  --input toolkit/phase-2-datos/control-de-calidad-de-arn-de-celula-unica/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Control de Calidad de ARN de Célula Única` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Control de Calidad de ARN de Célula Única",
