---
name: mejores-practicas-de-postgres
description: Guía oficial de Supabase que proporciona mejores prácticas de PostgreSQL, consejos de optimización y orientación arquitectónica para desarrolladores. Ayuda a mejorar el rendimiento de la base de datos, implementar una seguridad de nivel de fila RLS robusta y crear arquitecturas de datos más eficientes.
---

# Mejores prácticas de Postgres

## Propósito

Guía oficial de Supabase que proporciona mejores prácticas de PostgreSQL, consejos de optimización y orientación arquitectónica para desarrolladores. Ayuda a mejorar el rendimiento de la base de datos, implementar una seguridad de nivel de fila RLS robusta y crear arquitecturas de datos más eficientes. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/mejores-practicas-de-postgres/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `mejores-practicas-de-postgres` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/mejores-practicas-de-postgres/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/mejores-practicas-de-postgres/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/mejores-practicas-de-postgres/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/mejores-practicas-de-postgres/run.py \
  --input toolkit/phase-2-datos/mejores-practicas-de-postgres/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Mejores prácticas de Postgres` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Mejores prácticas de Postgres",
