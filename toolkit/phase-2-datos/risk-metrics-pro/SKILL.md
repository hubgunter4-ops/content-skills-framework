---
name: risk-metrics-pro
description: Herramienta de cálculo de riesgo de cartera que computa métricas principales como VaR, CVaR, ratio de Sharpe, ratio de Sortino y el drawdown máximo. Proporciona metodologías detalladas y mejores prácticas, ayudando a inversores cuantitativos y gestores de riesgos a medir con precisión el riesgo de la cartera, implementar límites y construir sistemas de monitoreo robustos.
---
...
Herramienta de cálculo de riesgo de cartera que computa métricas principales como VaR, CVaR, ratio de Sharpe, ratio de Sortino y el drawdown máximo. Proporciona metodologías detalladas y mejores prácticas, ayudando a inversores cuantitativos y gestores de riesgos a medir con precisión el riesgo de la cartera, implementar límites y construir sistemas de monitoreo robustos. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/risk-metrics-pro/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `risk-metrics-pro` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/risk-metrics-pro/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/risk-metrics-pro/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/risk-metrics-pro/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/risk-metrics-pro/run.py \
  --input toolkit/phase-2-datos/risk-metrics-pro/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Risk Metrics Pro` mediante el motor de la Fase 2.
