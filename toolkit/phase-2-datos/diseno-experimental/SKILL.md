---
name: diseno-experimental
description: Conjunto de herramientas de diseño experimental que ayuda a investigadores y analistas a planificar estudios proporcionando marcos para la aleatorización, el bloqueo y los diseños factoriales antes de la recopilación de datos. Convierte principios estadísticos complejos en planes ejecutables.
---
...
Conjunto de herramientas de diseño experimental que ayuda a investigadores y analistas a planificar estudios proporcionando marcos para la aleatorización, el bloqueo y los diseños factoriales antes de la recopilación de datos. Convierte principios estadísticos complejos en planes ejecutables. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/diseno-experimental/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `diseno-experimental` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/diseno-experimental/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/diseno-experimental/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/diseno-experimental/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/diseno-experimental/run.py \
  --input toolkit/phase-2-datos/diseno-experimental/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Diseño Experimental` mediante el motor de la Fase 2.
