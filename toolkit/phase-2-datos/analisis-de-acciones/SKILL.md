---
name: analisis-de-acciones
description: Utiliza datos de mercado, gráficos de precios, información sobre la propiedad y presentaciones ante la SEC para analizar acciones y empresas públicas.
---
...
Utiliza datos de mercado, gráficos de precios, información sobre la propiedad y presentaciones ante la SEC para analizar acciones y empresas públicas. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/analisis-de-acciones/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `analisis-de-acciones` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/analisis-de-acciones/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/analisis-de-acciones/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/analisis-de-acciones/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/analisis-de-acciones/run.py \
  --input toolkit/phase-2-datos/analisis-de-acciones/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Análisis de acciones` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Análisis de acciones",
