---
name: datos-fiscales-de-ee-uu
description: Herramienta que consulta la API de Datos Fiscales del Tesoro de los EE. UU. para recuperar datos financieros federales oficiales, incluyendo la deuda nacional, estados de cuenta del Tesoro y resultados de subastas, sin requerir una clave de API. Proporciona una fuente directa y autorizada para la investigación macroeconómica.
---

# Datos Fiscales de EE. UU.
...
- **Runner:** `toolkit/phase-2-datos/datos-fiscales-de-ee-uu/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `datos-fiscales-de-ee-uu` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/datos-fiscales-de-ee-uu/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/datos-fiscales-de-ee-uu/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/datos-fiscales-de-ee-uu/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/datos-fiscales-de-ee-uu/run.py \
  --input toolkit/phase-2-datos/datos-fiscales-de-ee-uu/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Datos Fiscales de EE. UU.` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Datos Fiscales de EE. UU.",
