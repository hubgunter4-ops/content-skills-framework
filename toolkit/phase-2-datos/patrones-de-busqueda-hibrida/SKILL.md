---
name: patrones-de-busqueda-hibrida
description: Proporciona patrones de implementación y plantillas de código para la búsqueda híbrida, combinando la búsqueda vectorial y por palabras clave mediante métodos como RRF y combinación lineal. Ayuda a los desarrolladores a crear sistemas RAG y motores de búsqueda más robustos al mejorar la recuperación de información cuando ninguno de los enfoques por sí solo es suficiente.
---

# Patrones de búsqueda híbrida

## Propósito

Proporciona patrones de implementación y plantillas de código para la búsqueda híbrida, combinando la búsqueda vectorial y por palabras clave mediante métodos como RRF y combinación lineal. Ayuda a los desarrolladores a crear sistemas RAG y motores de búsqueda más robustos al mejorar la recuperación de información cuando ninguno de los enfoques por sí solo es suficiente. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/patrones-de-busqueda-hibrida/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `patrones-de-busqueda-hibrida` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/patrones-de-busqueda-hibrida/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/patrones-de-busqueda-hibrida/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/patrones-de-busqueda-hibrida/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/patrones-de-busqueda-hibrida/run.py \
  --input toolkit/phase-2-datos/patrones-de-busqueda-hibrida/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Patrones de búsqueda híbrida` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Patrones de búsqueda híbrida",
