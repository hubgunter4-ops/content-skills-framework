---
name: agent-reach
description: Enrutador de contenido de internet unificado que permite a su agente buscar y leer en 15 plataformas — Xiaohongshu, Twitter/X, Bilibili, Reddit, YouTube, LinkedIn, RSS y más — a través de un conjunto de comandos consistente. Realice investigaciones multiplataforma y recopile discusiones de usuarios reales sin inventar trucos de scraping.
---
...
Enrutador de contenido de internet unificado que permite a su agente buscar y leer en 15 plataformas — Xiaohongshu, Twitter/X, Bilibili, Reddit, YouTube, LinkedIn, RSS y más — a través de un conjunto de comandos consistente. Realice investigaciones multiplataforma y recopile discusiones de usuarios reales sin inventar trucos de scraping. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/agent-reach/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `agent-reach` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/agent-reach/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/agent-reach/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/agent-reach/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/agent-reach/run.py \
  --input toolkit/phase-2-datos/agent-reach/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Agent Reach` mediante el motor de la Fase 2.
