---
name: redes-neuronales-de-grafos
description: Guía para PyTorch Geometric PyG que ayuda a los desarrolladores a construir, entrenar e interpretar redes neuronales de grafos para datos de grafos complejos. Cubre el paso de mensajes, grafos heterogéneos y conjuntos de datos personalizados, permitiendo a los equipos procesar estructuras de grafos y avanzar en su investigación de GNN.
---
...
Guía para PyTorch Geometric PyG que ayuda a los desarrolladores a construir, entrenar e interpretar redes neuronales de grafos para datos de grafos complejos. Cubre el paso de mensajes, grafos heterogéneos y conjuntos de datos personalizados, permitiendo a los equipos procesar estructuras de grafos y avanzar en su investigación de GNN. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/redes-neuronales-de-grafos/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `redes-neuronales-de-grafos` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/redes-neuronales-de-grafos/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/redes-neuronales-de-grafos/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/redes-neuronales-de-grafos/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/redes-neuronales-de-grafos/run.py \
  --input toolkit/phase-2-datos/redes-neuronales-de-grafos/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Redes Neuronales de Grafos` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Redes Neuronales de Grafos",
