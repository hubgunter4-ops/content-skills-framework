---
name: escalador-de-datos-dask
description: Guía de computación distribuida para Dask, que cubre las mejores prácticas para DataFrames, Arrays, Bags y Futures. Ayuda a escalar los flujos de trabajo existentes de pandas y NumPy más allá de los límites de memoria de una sola máquina, facilitando el procesamiento de conjuntos de datos masivos, la ejecución de canalizaciones ETL paralelas y la optimización del rendimiento en clústeres.
---

# Escalador de Datos Dask

## Propósito

Guía de computación distribuida para Dask, que cubre las mejores prácticas para DataFrames, Arrays, Bags y Futures. Ayuda a escalar los flujos de trabajo existentes de pandas y NumPy más allá de los límites de memoria de una sola máquina, facilitando el procesamiento de conjuntos de datos masivos, la ejecución de canalizaciones ETL paralelas y la optimización del rendimiento en clústeres. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/escalador-de-datos-dask/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `escalador-de-datos-dask` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/escalador-de-datos-dask/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/escalador-de-datos-dask/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/escalador-de-datos-dask/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/escalador-de-datos-dask/run.py \
  --input toolkit/phase-2-datos/escalador-de-datos-dask/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Escalador de Datos Dask` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Escalador de Datos Dask",
