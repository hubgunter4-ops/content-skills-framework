---
name: explicador-shap
description: Herramienta de interpretabilidad de modelos de aprendizaje automático que utiliza SHAP para facilitar la selección de explicadores y generar visualizaciones como gráficos de cascada waterfall y de abejas beeswarm. Traduce predicciones complejas en un análisis intuitivo de la importancia de las características.
---
...
Herramienta de interpretabilidad de modelos de aprendizaje automático que utiliza SHAP para facilitar la selección de explicadores y generar visualizaciones como gráficos de cascada waterfall y de abejas beeswarm. Traduce predicciones complejas en un análisis intuitivo de la importancia de las características. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/explicador-shap/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `explicador-shap` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/explicador-shap/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/explicador-shap/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/explicador-shap/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/explicador-shap/run.py \
  --input toolkit/phase-2-datos/explicador-shap/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Explicador SHAP` mediante el motor de la Fase 2.
