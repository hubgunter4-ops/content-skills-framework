---
name: extractor-de-contexto-de-datos
description: Ayuda a generar o mejorar una capacidad de análisis de datos específica de la empresa mediante la extracción del conocimiento tribal de los analistas, el descubrimiento de esquemas y la producción de archivos de referencia y patrones de consulta.
---

# Extractor de contexto de datos

## Propósito

Ayuda a generar o mejorar una capacidad de análisis de datos específica de la empresa mediante la extracción del conocimiento tribal de los analistas, el descubrimiento de esquemas y la producción de archivos de referencia y patrones de consulta. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/extractor-de-contexto-de-datos/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `extractor-de-contexto-de-datos` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/extractor-de-contexto-de-datos/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/extractor-de-contexto-de-datos/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/extractor-de-contexto-de-datos/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/extractor-de-contexto-de-datos/run.py \
  --input toolkit/phase-2-datos/extractor-de-contexto-de-datos/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Extractor de contexto de datos` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Extractor de contexto de datos",
