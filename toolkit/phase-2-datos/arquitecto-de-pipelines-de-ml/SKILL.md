---
name: arquitecto-de-pipelines-de-ml
description: Guía de ingeniería de pipelines de ML de nivel de producción que cubre el seguimiento de experimentos con MLflow o Weights & Biases, la orquestación de entrenamiento con Kubeflow o Airflow DAGs, la configuración de almacenes de características con Feast y el reentrenamiento y validación automatizados de modelos. Estandariza el flujo de trabajo de MLOps de extremo a extremo.
---

# Arquitecto de Pipelines de ML

## Propósito

Guía de ingeniería de pipelines de ML de nivel de producción que cubre el seguimiento de experimentos con MLflow o Weights & Biases, la orquestación de entrenamiento con Kubeflow o Airflow DAGs, la configuración de almacenes de características con Feast y el reentrenamiento y validación automatizados de modelos. Estandariza el flujo de trabajo de MLOps de extremo a extremo. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/arquitecto-de-pipelines-de-ml/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `arquitecto-de-pipelines-de-ml` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/arquitecto-de-pipelines-de-ml/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/arquitecto-de-pipelines-de-ml/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/arquitecto-de-pipelines-de-ml/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/arquitecto-de-pipelines-de-ml/run.py \
  --input toolkit/phase-2-datos/arquitecto-de-pipelines-de-ml/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Arquitecto de Pipelines de ML` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Arquitecto de Pipelines de ML",
