---
name: constructor-de-paneles-interactivos
description: Crea paneles HTML interactivos e independientes utilizando Chart.js, filtros desplegables y un estilo profesional. Genera archivos HTML compartibles y con capacidad offline que muestran gráficos, permiten el filtrado dinámico, admiten diseños adaptables y no requieren servidor.
---
...
Crea paneles HTML interactivos e independientes utilizando Chart.js, filtros desplegables y un estilo profesional. Genera archivos HTML compartibles y con capacidad offline que muestran gráficos, permiten el filtrado dinámico, admiten diseños adaptables y no requieren servidor. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/constructor-de-paneles-interactivos/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `constructor-de-paneles-interactivos` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/constructor-de-paneles-interactivos/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/constructor-de-paneles-interactivos/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/constructor-de-paneles-interactivos/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/constructor-de-paneles-interactivos/run.py \
  --input toolkit/phase-2-datos/constructor-de-paneles-interactivos/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Constructor de Paneles Interactivos` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Constructor de Paneles Interactivos",
