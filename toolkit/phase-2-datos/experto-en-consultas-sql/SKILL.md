---
name: experto-en-consultas-sql
description: Herramienta que genera SQL correcto y eficiente en los principales dialectos de almacenes de datos Snowflake, BigQuery, Databricks, PostgreSQL, etc.. Ayuda a escribir consultas analíticas, traducir sintaxis específica de dialectos y optimizar consultas lentas mediante el uso de CTE, funciones de ventana y mejores prácticas de agregación.
---
...
Herramienta que genera SQL correcto y eficiente en los principales dialectos de almacenes de datos Snowflake, BigQuery, Databricks, PostgreSQL, etc.. Ayuda a escribir consultas analíticas, traducir sintaxis específica de dialectos y optimizar consultas lentas mediante el uso de CTE, funciones de ventana y mejores prácticas de agregación. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/experto-en-consultas-sql/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `experto-en-consultas-sql` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/experto-en-consultas-sql/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/experto-en-consultas-sql/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/experto-en-consultas-sql/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/experto-en-consultas-sql/run.py \
  --input toolkit/phase-2-datos/experto-en-consultas-sql/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Experto en Consultas SQL` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Experto en Consultas SQL",
