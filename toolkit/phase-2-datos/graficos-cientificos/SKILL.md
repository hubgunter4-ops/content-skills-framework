---
name: graficos-cientificos
description: Guía de la biblioteca de gráficos Matplotlib que cubre gráficos básicos, subgráficos múltiples complejos y visualizaciones en 3D. Proporciona un control detallado sobre los elementos del gráfico, ayudando a los usuarios a crear figuras personalizadas con calidad de publicación y exportarlas en formatos PNG, PDF o SVG.
---
...
Guía de la biblioteca de gráficos Matplotlib que cubre gráficos básicos, subgráficos múltiples complejos y visualizaciones en 3D. Proporciona un control detallado sobre los elementos del gráfico, ayudando a los usuarios a crear figuras personalizadas con calidad de publicación y exportarlas en formatos PNG, PDF o SVG. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/graficos-cientificos/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `graficos-cientificos` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/graficos-cientificos/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/graficos-cientificos/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/graficos-cientificos/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/graficos-cientificos/run.py \
  --input toolkit/phase-2-datos/graficos-cientificos/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Gráficos científicos` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Gráficos científicos",
