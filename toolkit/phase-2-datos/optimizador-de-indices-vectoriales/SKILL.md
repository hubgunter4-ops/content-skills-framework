---
name: optimizador-de-indices-vectoriales
description: Proporciona una guía de nivel de producción y plantillas de código para optimizar el rendimiento de los índices vectoriales, abarcando el ajuste de parámetros HNSW, estrategias de cuantificación y monitoreo del rendimiento. Ayuda a los desarrolladores que manejan búsquedas vectoriales a gran escala a equilibrar la latencia, la recuperación y el uso de memoria.
---

# Optimizador de Índices Vectoriales

## Propósito

Proporciona una guía de nivel de producción y plantillas de código para optimizar el rendimiento de los índices vectoriales, abarcando el ajuste de parámetros HNSW, estrategias de cuantificación y monitoreo del rendimiento. Ayuda a los desarrolladores que manejan búsquedas vectoriales a gran escala a equilibrar la latencia, la recuperación y el uso de memoria. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/optimizador-de-indices-vectoriales/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `optimizador-de-indices-vectoriales` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/optimizador-de-indices-vectoriales/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/optimizador-de-indices-vectoriales/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/optimizador-de-indices-vectoriales/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/optimizador-de-indices-vectoriales/run.py \
  --input toolkit/phase-2-datos/optimizador-de-indices-vectoriales/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Optimizador de Índices Vectoriales` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Optimizador de Índices Vectoriales",
