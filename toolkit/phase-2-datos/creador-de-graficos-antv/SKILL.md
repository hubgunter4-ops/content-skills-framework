---
name: creador-de-graficos-antv
description: Herramienta oficial de generación de gráficos de AntV que ofrece más de 26 tipos de gráficos, desde gráficos de barras y líneas hasta diagramas de Sankey, mapas mentales y nubes de palabras. Selecciona el mejor gráfico para sus datos y devuelve una imagen del gráfico lista para compartir a través de la API de AntV en segundos.
---

# Creador de gráficos AntV

## Propósito

Herramienta oficial de generación de gráficos de AntV que ofrece más de 26 tipos de gráficos, desde gráficos de barras y líneas hasta diagramas de Sankey, mapas mentales y nubes de palabras. Selecciona el mejor gráfico para sus datos y devuelve una imagen del gráfico lista para compartir a través de la API de AntV en segundos. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/creador-de-graficos-antv/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `creador-de-graficos-antv` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/creador-de-graficos-antv/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/creador-de-graficos-antv/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/creador-de-graficos-antv/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/creador-de-graficos-antv/run.py \
  --input toolkit/phase-2-datos/creador-de-graficos-antv/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Creador de gráficos AntV` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Creador de gráficos AntV",
