---
name: analitica-de-big-data
description: Guía de procesamiento de big data que ayuda a utilizar la biblioteca Vaex para analizar y visualizar conjuntos de datos tabulares masivos, como archivos CSV o HDF5 de varios gigabytes. Al aprovechar las operaciones fuera del núcleo out-of-core y la evaluación diferida lazy evaluation, permite realizar estadísticas rápidas, filtrado y aprendizaje automático en miles de millones de filas sin quedarse sin memoria.
---
...
Guía de procesamiento de big data que ayuda a utilizar la biblioteca Vaex para analizar y visualizar conjuntos de datos tabulares masivos, como archivos CSV o HDF5 de varios gigabytes. Al aprovechar las operaciones fuera del núcleo out-of-core y la evaluación diferida lazy evaluation, permite realizar estadísticas rápidas, filtrado y aprendizaje automático en miles de millones de filas sin quedarse sin memoria. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/analitica-de-big-data/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `analitica-de-big-data` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/analitica-de-big-data/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/analitica-de-big-data/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/analitica-de-big-data/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/analitica-de-big-data/run.py \
  --input toolkit/phase-2-datos/analitica-de-big-data/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Analítica de Big Data` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Analítica de Big Data",
