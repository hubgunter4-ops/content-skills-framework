---
name: mongo-query-pro
description: Herramienta que traduce solicitudes en lenguaje natural en consultas de MongoDB y pipelines de agregación ejecutables. Cierra la brecha entre el texto plano y las operaciones complejas de bases de datos, ayudando a desarrolladores y analistas a recuperar datos más rápido sin necesidad de escribir o depurar manualmente la intrincada sintaxis de MongoDB.
---
...
Herramienta que traduce solicitudes en lenguaje natural en consultas de MongoDB y pipelines de agregación ejecutables. Cierra la brecha entre el texto plano y las operaciones complejas de bases de datos, ayudando a desarrolladores y analistas a recuperar datos más rápido sin necesidad de escribir o depurar manualmente la intrincada sintaxis de MongoDB. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/mongo-query-pro/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `mongo-query-pro` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/mongo-query-pro/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/mongo-query-pro/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/mongo-query-pro/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/mongo-query-pro/run.py \
  --input toolkit/phase-2-datos/mongo-query-pro/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Mongo Query Pro` mediante el motor de la Fase 2.
