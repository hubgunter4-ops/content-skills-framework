---
name: potencia-estadistica-pro
description: Herramienta que calcula el tamaño de la muestra, la potencia estadística, el efecto mínimo detectable y las curvas de potencia para experimentos, estudios y protocolos clínicos en diseños estándar y basados en simulación. Ayuda a los investigadores a justificar planes de estudio con evidencia cuantitativa defendible, reducir el trabajo con baja potencia y preparar propuestas, materiales para el IRB y prerregistros más sólidos.
---

# Potencia Estadística Pro

## Propósito

Herramienta que calcula el tamaño de la muestra, la potencia estadística, el efecto mínimo detectable y las curvas de potencia para experimentos, estudios y protocolos clínicos en diseños estándar y basados en simulación. Ayuda a los investigadores a justificar planes de estudio con evidencia cuantitativa defendible, reducir el trabajo con baja potencia y preparar propuestas, materiales para el IRB y prerregistros más sólidos. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/potencia-estadistica-pro/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `potencia-estadistica-pro` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/potencia-estadistica-pro/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/potencia-estadistica-pro/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/potencia-estadistica-pro/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/potencia-estadistica-pro/run.py \
  --input toolkit/phase-2-datos/potencia-estadistica-pro/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Potencia Estadística Pro` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Potencia Estadística Pro",
