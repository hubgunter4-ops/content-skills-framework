---
name: entrenador-de-modelos-de-rl
description: Biblioteca que ofrece algoritmos de aprendizaje por refuerzo listos para producción como PPO y SAC con una API similar a scikit-learn para experimentos y prototipado estándar de RL. Agiliza la implementación y evaluación de algoritmos, permitiendo un desarrollo más rápido de modelos de RL de un solo agente dentro de entornos de Gymnasium.
---

# Entrenador de Modelos de RL

## Propósito

Biblioteca que ofrece algoritmos de aprendizaje por refuerzo listos para producción como PPO y SAC con una API similar a scikit-learn para experimentos y prototipado estándar de RL. Agiliza la implementación y evaluación de algoritmos, permitiendo un desarrollo más rápido de modelos de RL de un solo agente dentro de entornos de Gymnasium. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/entrenador-de-modelos-de-rl/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `entrenador-de-modelos-de-rl` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/entrenador-de-modelos-de-rl/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/entrenador-de-modelos-de-rl/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/entrenador-de-modelos-de-rl/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/entrenador-de-modelos-de-rl/run.py \
  --input toolkit/phase-2-datos/entrenador-de-modelos-de-rl/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Entrenador de Modelos de RL` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Entrenador de Modelos de RL",
