---
name: motor-de-datos-polars
description: Guía de la biblioteca DataFrame de alto rendimiento que cubre conceptos básicos, optimización de consultas perezosas lazy y migración de pandas para ETL y analítica. Ayuda a procesar conjuntos de datos masivos de manera eficiente con ejecución paralela y procesamiento fuera del núcleo out-of-core mediante streaming.
---
...
Guía de la biblioteca DataFrame de alto rendimiento que cubre conceptos básicos, optimización de consultas perezosas lazy y migración de pandas para ETL y analítica. Ayuda a procesar conjuntos de datos masivos de manera eficiente con ejecución paralela y procesamiento fuera del núcleo out-of-core mediante streaming. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/motor-de-datos-polars/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `motor-de-datos-polars` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/motor-de-datos-polars/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/motor-de-datos-polars/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/motor-de-datos-polars/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/motor-de-datos-polars/run.py \
  --input toolkit/phase-2-datos/motor-de-datos-polars/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Motor de Datos Polars` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Motor de Datos Polars",
