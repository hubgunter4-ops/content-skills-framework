...
description: Crea hojas de cálculo de Excel profesionales con análisis de datos estructurado. Organiza, analiza y presenta datos empresariales y financieros en formatos limpios y listos para su publicación.
---
...
Crea hojas de cálculo de Excel profesionales con análisis de datos estructurado. Organiza, analiza y presenta datos empresariales y financieros en formatos limpios y listos para su publicación. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/generador-de-excel/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `generador-de-excel` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/generador-de-excel/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/generador-de-excel/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/generador-de-excel/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/generador-de-excel/run.py \
  --input toolkit/phase-2-datos/generador-de-excel/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Generador de Excel` mediante el motor de la Fase 2.
