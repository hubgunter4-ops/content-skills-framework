...
description: Investiga las finanzas de las empresas, los precios de mercado, las presentaciones ante la SEC, las conferencias de resultados y los datos macroeconómicos utilizando APIs financieras estructuradas.
---
...
Investiga las finanzas de las empresas, los precios de mercado, las presentaciones ante la SEC, las conferencias de resultados y los datos macroeconómicos utilizando APIs financieras estructuradas. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/analisis-financiero/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `analisis-financiero` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/analisis-financiero/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/analisis-financiero/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/analisis-financiero/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/analisis-financiero/run.py \
  --input toolkit/phase-2-datos/analisis-financiero/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Análisis Financiero` mediante el motor de la Fase 2.
