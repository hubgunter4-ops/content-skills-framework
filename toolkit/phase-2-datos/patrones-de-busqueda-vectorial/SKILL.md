...
Kit de herramientas que proporciona mejores prácticas y plantillas de código para implementar búsquedas de similitud eficientes en bases de datos vectoriales, incluyendo búsqueda híbrida y optimización del rendimiento. Ayuda a los desarrolladores a crear búsquedas semánticas y consultas de vecinos más cercanos más rápido. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/patrones-de-busqueda-vectorial/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `patrones-de-busqueda-vectorial` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/patrones-de-busqueda-vectorial/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/patrones-de-busqueda-vectorial/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/patrones-de-busqueda-vectorial/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/patrones-de-busqueda-vectorial/run.py \
  --input toolkit/phase-2-datos/patrones-de-busqueda-vectorial/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Patrones de búsqueda vectorial` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Patrones de búsqueda vectorial",
