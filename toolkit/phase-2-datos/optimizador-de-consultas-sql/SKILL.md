...
Manual de optimización de SQL probado en combate que cubre el análisis de planes EXPLAIN, estrategias de indexación y patrones de reescritura de consultas para PostgreSQL y más allá. Ayuda a convertir consultas lentas en otras ultrarrápidas, reducir la carga de la base de datos y diseñar esquemas que escalen con tus datos. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/optimizador-de-consultas-sql/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `optimizador-de-consultas-sql` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/optimizador-de-consultas-sql/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/optimizador-de-consultas-sql/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/optimizador-de-consultas-sql/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/optimizador-de-consultas-sql/run.py \
  --input toolkit/phase-2-datos/optimizador-de-consultas-sql/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Optimizador de Consultas SQL` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Optimizador de Consultas SQL",
