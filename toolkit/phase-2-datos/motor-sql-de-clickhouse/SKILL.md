---
name: motor-sql-de-clickhouse
description: Motor SQL de ClickHouse integrado para Python que permite consultar archivos, bases de datos y almacenamiento en la nube sin un servidor dedicado. Permite una ejecución de SQL rápida y sin servidor directamente dentro de su entorno local, agilizando los flujos de trabajo de análisis de datos y mejorando la eficiencia del procesamiento.
---

# Motor SQL de ClickHouse

## Propósito

Motor SQL de ClickHouse integrado para Python que permite consultar archivos, bases de datos y almacenamiento en la nube sin un servidor dedicado. Permite una ejecución de SQL rápida y sin servidor directamente dentro de su entorno local, agilizando los flujos de trabajo de análisis de datos y mejorando la eficiencia del procesamiento. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/motor-sql-de-clickhouse/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `motor-sql-de-clickhouse` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/motor-sql-de-clickhouse/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/motor-sql-de-clickhouse/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/motor-sql-de-clickhouse/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/motor-sql-de-clickhouse/run.py \
  --input toolkit/phase-2-datos/motor-sql-de-clickhouse/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Motor SQL de ClickHouse` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Motor SQL de ClickHouse",
