---
name: buscador-de-datos-cientificos
description: Herramienta de búsqueda en bases de datos que ayuda a los usuarios a consultar con precisión 78 bases de datos públicas en las áreas de biología, química y finanzas mediante el uso de APIs documentadas. Proporciona datos científicos y estadísticos autorizados y reproducibles.
---

# Buscador de Datos Científicos

## Propósito

Herramienta de búsqueda en bases de datos que ayuda a los usuarios a consultar con precisión 78 bases de datos públicas en las áreas de biología, química y finanzas mediante el uso de APIs documentadas. Proporciona datos científicos y estadísticos autorizados y reproducibles. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/buscador-de-datos-cientificos/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `buscador-de-datos-cientificos` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/buscador-de-datos-cientificos/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/buscador-de-datos-cientificos/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/buscador-de-datos-cientificos/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/buscador-de-datos-cientificos/run.py \
  --input toolkit/phase-2-datos/buscador-de-datos-cientificos/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Buscador de Datos Científicos` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Buscador de Datos Científicos",
