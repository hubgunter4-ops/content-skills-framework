---
name: extractor-de-hacker-news
description: Herramienta que extrae la página principal de Hacker News y devuelve las 30 historias principales como JSON estructurado, incluyendo rank, title, URL, points y recuentos de comentarios. Proporciona una fuente rápida y legible por máquina para monitoreo, paneles, boletines o análisis automatizados sin necesidad de copiar o procesar manualmente.
---

# Extractor de Hacker News

## Propósito

Herramienta que extrae la página principal de Hacker News y devuelve las 30 historias principales como JSON estructurado, incluyendo rank, title, URL, points y recuentos de comentarios. Proporciona una fuente rápida y legible por máquina para monitoreo, paneles, boletines o análisis automatizados sin necesidad de copiar o procesar manualmente. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/extractor-de-hacker-news/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `extractor-de-hacker-news` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/extractor-de-hacker-news/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/extractor-de-hacker-news/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/extractor-de-hacker-news/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/extractor-de-hacker-news/run.py \
  --input toolkit/phase-2-datos/extractor-de-hacker-news/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Extractor de Hacker News` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Extractor de Hacker News",
