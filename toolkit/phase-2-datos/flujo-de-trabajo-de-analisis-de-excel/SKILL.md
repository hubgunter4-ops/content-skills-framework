---
name: flujo-de-trabajo-de-analisis-de-excel
description: Orquestador de análisis de datos de Excel de extremo a extremo que cubre la lectura de múltiples hojas, limpieza, filtrado, agregación y exportación formateada. Guía todo el proceso paso a paso para obtener informes de análisis fiables y descargables.
---

# Flujo de trabajo de análisis de Excel

## Propósito

Orquestador de análisis de datos de Excel de extremo a extremo que cubre la lectura de múltiples hojas, limpieza, filtrado, agregación y exportación formateada. Guía todo el proceso paso a paso para obtener informes de análisis fiables y descargables. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/flujo-de-trabajo-de-analisis-de-excel/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `flujo-de-trabajo-de-analisis-de-excel` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/flujo-de-trabajo-de-analisis-de-excel/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/flujo-de-trabajo-de-analisis-de-excel/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/flujo-de-trabajo-de-analisis-de-excel/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/flujo-de-trabajo-de-analisis-de-excel/run.py \
  --input toolkit/phase-2-datos/flujo-de-trabajo-de-analisis-de-excel/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Flujo de trabajo de análisis de Excel` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Flujo de trabajo de análisis de Excel",
