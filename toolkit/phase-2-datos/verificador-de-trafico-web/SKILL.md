---
name: verificador-de-trafico-web
description: Flujo de trabajo de análisis integral del tráfico web compatible con Similarweb, Ahrefs, Semrush, DataForSEO y datos de analítica exportados.
---

# Verificador de tráfico web

## Propósito

Flujo de trabajo de análisis integral del tráfico web compatible con Similarweb, Ahrefs, Semrush, DataForSEO y datos de analítica exportados. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/verificador-de-trafico-web/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `verificador-de-trafico-web` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/verificador-de-trafico-web/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/verificador-de-trafico-web/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/verificador-de-trafico-web/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/verificador-de-trafico-web/run.py \
  --input toolkit/phase-2-datos/verificador-de-trafico-web/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Verificador de tráfico web` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Verificador de tráfico web",
