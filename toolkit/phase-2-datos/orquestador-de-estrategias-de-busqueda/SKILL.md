---
name: orquestador-de-estrategias-de-busqueda
description: Orquesta la descomposición de consultas y los flujos de trabajo de búsqueda en múltiples fuentes para recuperar respuestas precisas de diversos almacenes de datos. Divide las preguntas en lenguaje natural en subconsultas específicas, las traduce a la sintaxis propia de cada fuente, clasifica y consolida los resultados por relevancia.
---
...
Orquesta la descomposición de consultas y los flujos de trabajo de búsqueda en múltiples fuentes para recuperar respuestas precisas de diversos almacenes de datos. Divide las preguntas en lenguaje natural en subconsultas específicas, las traduce a la sintaxis propia de cada fuente, clasifica y consolida los resultados por relevancia. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/orquestador-de-estrategias-de-busqueda/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `orquestador-de-estrategias-de-busqueda` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/orquestador-de-estrategias-de-busqueda/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/orquestador-de-estrategias-de-busqueda/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/orquestador-de-estrategias-de-busqueda/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/orquestador-de-estrategias-de-busqueda/run.py \
  --input toolkit/phase-2-datos/orquestador-de-estrategias-de-busqueda/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Orquestador de Estrategias de Búsqueda` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Orquestador de Estrategias de Búsqueda",
