---
name: chdb-data-store
description: Herramienta de almacenamiento de datos que proporciona un reemplazo directo para pandas, ofreciendo un rendimiento al nivel de ClickHouse en más de 16 fuentes de datos. Permite a los equipos consultar y analizar grandes conjuntos de datos de manera eficiente, optimizando los flujos de trabajo de datos sin necesidad de una configuración de infraestructura compleja.
---
...
Herramienta de almacenamiento de datos que proporciona un reemplazo directo para pandas, ofreciendo un rendimiento al nivel de ClickHouse en más de 16 fuentes de datos. Permite a los equipos consultar y analizar grandes conjuntos de datos de manera eficiente, optimizando los flujos de trabajo de datos sin necesidad de una configuración de infraestructura compleja. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/chdb-data-store/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `chdb-data-store` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/chdb-data-store/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/chdb-data-store/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/chdb-data-store/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/chdb-data-store/run.py \
  --input toolkit/phase-2-datos/chdb-data-store/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `chDB Data Store` mediante el motor de la Fase 2.
