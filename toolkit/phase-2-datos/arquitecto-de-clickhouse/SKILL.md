...
description: Asesor de arquitectura que ayuda a diseñar implementaciones de ClickHouse y a traducir las mejores prácticas en decisiones específicas para su carga de trabajo. Proporciona orientación experta sobre el diseño de clústeres, la optimización de esquemas y el rendimiento de las consultas.
---
...
Asesor de arquitectura que ayuda a diseñar implementaciones de ClickHouse y a traducir las mejores prácticas en decisiones específicas para su carga de trabajo. Proporciona orientación experta sobre el diseño de clústeres, la optimización de esquemas y el rendimiento de las consultas. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/arquitecto-de-clickhouse/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `arquitecto-de-clickhouse` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/arquitecto-de-clickhouse/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/arquitecto-de-clickhouse/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/arquitecto-de-clickhouse/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/arquitecto-de-clickhouse/run.py \
  --input toolkit/phase-2-datos/arquitecto-de-clickhouse/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Arquitecto de ClickHouse` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Arquitecto de ClickHouse",
