---
name: revision-de-tabla-de-contratos
description: Convierte pilas de acuerdos comerciales en una tabla comparativa estructurada con 19 columnas de extracción predefinidas que cubren las partes, la vigencia, los precios, los límites de responsabilidad y más. Cargue sus contratos y obtenga una revisión de Excel limpia y exportable en minutos.
---

# Revisión de tabla de contratos

## Propósito

Convierte pilas de acuerdos comerciales en una tabla comparativa estructurada con 19 columnas de extracción predefinidas que cubren las partes, la vigencia, los precios, los límites de responsabilidad y más. Cargue sus contratos y obtenga una revisión de Excel limpia y exportable en minutos. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/revision-de-tabla-de-contratos/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `revision-de-tabla-de-contratos` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/revision-de-tabla-de-contratos/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/revision-de-tabla-de-contratos/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/revision-de-tabla-de-contratos/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/revision-de-tabla-de-contratos/run.py \
  --input toolkit/phase-2-datos/revision-de-tabla-de-contratos/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Revisión de tabla de contratos` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Revisión de tabla de contratos",
