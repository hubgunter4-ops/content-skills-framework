...
Proporciona patrones de transformación de datos dbt de grado de producción, que cubren la organización de modelos, pruebas de calidad, estrategias incrementales y estándares de documentación. Ayuda a los equipos de ingeniería de datos a construir pipelines de datos eficientes, implementar mejores prácticas de analítica y mantener modelos de datos claros y escalables. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/patrones-de-transformacion-de-dbt/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `patrones-de-transformacion-de-dbt` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/patrones-de-transformacion-de-dbt/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/patrones-de-transformacion-de-dbt/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/patrones-de-transformacion-de-dbt/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/patrones-de-transformacion-de-dbt/run.py \
  --input toolkit/phase-2-datos/patrones-de-transformacion-de-dbt/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Patrones de transformación de dbt` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Patrones de transformación de dbt",
