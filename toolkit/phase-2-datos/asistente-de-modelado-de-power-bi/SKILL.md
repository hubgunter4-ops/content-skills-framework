---
name: asistente-de-modelado-de-power-bi
description: Herramienta que se conecta a su modelo semántico activo de Power BI a través de las utilidades MCP de power-bi-modeling y ofrece orientación práctica orientada al modelo. Ayuda a diseñar esquemas en estrella, crear medidas DAX eficientes, configurar relaciones y RLS, definir grupos de cálculo y aplicar mejores prácticas de rendimiento y nomenclatura.
---

# Asistente de Modelado de Power BI

## Propósito

Herramienta que se conecta a su modelo semántico activo de Power BI a través de las utilidades MCP de power-bi-modeling y ofrece orientación práctica orientada al modelo. Ayuda a diseñar esquemas en estrella, crear medidas DAX eficientes, configurar relaciones y RLS, definir grupos de cálculo y aplicar mejores prácticas de rendimiento y nomenclatura. Esta herramienta prepara un entregable local, estructurado y revisable a partir de la información que proporcione el usuario. No sustituye una investigación de dominio ni convierte una plantilla en evidencia ejecutada.

## Runner asociado

- **Runner:** `toolkit/phase-2-datos/asistente-de-modelado-de-power-bi/run.py`
- **Motor:** `toolkit/phase_2_engine.py`
- **Adaptador:** rama de `build_content()` asociada a `asistente-de-modelado-de-power-bi` o adaptador de datos genérico cuando no existe una rama específica.
- **Entrada de ejemplo:** `toolkit/phase-2-datos/asistente-de-modelado-de-power-bi/input.example.json`
- **Esquema de salida:** `toolkit/phase-2-datos/asistente-de-modelado-de-power-bi/output.schema.json`
- **Prueba smoke:** `toolkit/phase-2-datos/asistente-de-modelado-de-power-bi/tests/test_smoke.py`

La ejecución directa se realiza con:

```bash
python3 toolkit/phase-2-datos/asistente-de-modelado-de-power-bi/run.py \
  --input toolkit/phase-2-datos/asistente-de-modelado-de-power-bi/input.example.json
```
...
4. **Aplicar el adaptador local.** Generar el formato o análisis que corresponda a `Asistente de Modelado de Power BI` mediante el motor de la Fase 2.
...
```json
{
  "objective": "Asistente de Modelado de Power BI",
