...
def build(skill: dict[str, str], payload: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if 'git' in low or 'commit' in low or 'rama' in low:
        options = '- Crear commit atómico.\n- Abrir pull request.\n- Fusionar según política del repositorio.\n- Limpiar rama solo con confirmación.'
        verification = 'Ejecutar pruebas, revisar diff, confirmar mensaje y comprobar el SHA remoto.'
        return f"# Flujo Git\n\n{section('Estado esperado', 'Revisar estado, diff, rama, upstream y reglas de contribución.')}{section('Opciones', options)}{section('Verificación', verification)}", ['No se ejecutaron comandos Git de modificación desde esta herramienta.']
