...
def build(skill: dict[str, str], payload: dict[str, Any]) -> tuple[str, list[str]]:
    ...
    if 'accesibilidad' in low:
        wcag = '- Contraste de texto y componentes.\n- Navegación completa por teclado y foco visible.\n- Objetivos táctiles y zoom.\n- Nombres accesibles, roles y estados.\n- Mensajes de error y orden de lectura.\n- Preferencia `prefers-reduced-motion`.'
        report = 'Priorizar por impacto, frecuencia, criterio WCAG, evidencia, corrección y prueba de regresión.'
        return f"# Auditoría de accesibilidad\n\n{sec('Entrada', text or '[HTML, URL o código pendiente]')}{sec('WCAG 2.2', wcag)}{sec('Informe', report)}", ['Ejecutar pruebas con teclado, lector de pantalla y herramientas automatizadas autorizadas.']
    if 'paleta' in low:
    ...
    if 'sistema de diseño' in low or 'tailwind' in low or 'nothing' in low or 'marca' in low or 'word' in low:
        ...
        return f"# {name}\n\n{sec('Principios', 'Definir tokens de color, tipografía, espaciado, radios, sombras, estados y componentes.')}{sec('Entrada', text or '[Guía de marca o template pendiente]')}{sec('Tokens iniciales', tokens)}{sec('Aplicación', 'Mantener una fuente de verdad, documentar variantes y probar contraste, responsive y estados.')}", ['Validar tokens contra la marca y los componentes reales.']
